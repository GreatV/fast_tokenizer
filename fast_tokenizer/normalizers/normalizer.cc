...
void NormalizedString::UpdateNormalizedRange(
    const OffsetMapping& new_normalized, uint32_t initial_offset,
    const core::Range& range, bool origin_range) {
  auto n_range = range;
  if (origin_range) {
  ...
  if (n_range.second - n_range.first >= alignments.size()) {
    std::memcpy(alignments_.data() + n_range.first, alignments.data(),
    ...
    std::copy_n(alignments.begin(), alignments.size(),
                new_alignments.begin() + n_range.first);
    if (third_len > 0) {
      std::copy_n(alignments_.begin() + n_range.second, third_len,
  ...
  utils::GetUTF8Str(new_normalized.u32normalized.data(), utf8_str.data(),
  ...
  normalized_.replace(normalized_iter + n_range.first,
                      normalized_iter + n_range.second, utf8_str.data(),
...
void NormalizedString::RunNormalization(const std::string& mode) {
  ...
  normalizer->normalizeUTF8(
      0, icu::StringPiece(normalized_.data(), normalized_.size()), byte_sink,
      &edits, icu_error);
  std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t> conv;
...
NormalizedString& NormalizedString::Replace(const re2::RE2& pattern,
  ...
  while (pattern.Match(normalized_, start, end, RE2::UNANCHORED, &result, 1)) {
    ...
    UpdateNormalizedRange(new_normalized, removed_chars, {curr_start, curr_end},
                          false);
...
bool NormalizedString::Slice(core::Range range, NormalizedString* normalized,
                             bool origin_range) const {
...
uint32_t NormalizedString::GetMatch(
    const std::string& normalized, const re2::RE2& pattern,
    std::vector<std::pair<core::Range, bool>>* matches, bool invert) const {
  size_t start = 0;
...
template void NormalizedString::Split(
    const std::function<bool(char32_t)>& pattern_func, core::SplitMode mode,
    std::vector<NormalizedString>* normalizes, bool invert) const;
