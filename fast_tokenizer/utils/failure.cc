...
FailureVocabToken::FailureVocabToken(
    const std::string& token, int token_id,
...
void FailureArray::BuildFailureVocab(
  ...
  for (auto& item : vocab) {
    ...
    FailureVocabToken vocab_token(item.first, item.second,
  ...
  if (!continuing_subword_prefix.empty()) {
    const bool suffix_token_exists = std::any_of(
        failure_vocab_tokens_.begin(), failure_vocab_tokens_.end(),
    ...
    if (!suffix_token_exists) {
      ...
      failure_vocab_tokens_.emplace_back(new_suffix_token, unk_id,
                                         continuing_subword_prefix);
    }
  }
  if (with_pretokenization_) {
    for (uint32_t cp = 1; cp <= 0x0010FFFF; ++cp) {
      ...
      if (vocab.count(punc_str) == 0) {
        failure_vocab_tokens_.emplace_back(punc_str, unk_id,
                                           continuing_subword_prefix);
      }
    }
    failure_vocab_tokens_.emplace_back(std::string(1, kInvalidControlChar),
                                       unk_id, continuing_subword_prefix);
  }
}
...
void FailureArray::InitFromVocabAndTrie(
    const std::unordered_map<std::string, uint32_t>& vocab, Trie* trie,
    const std::string& unk_token,
    const std::string& continuing_subword_prefix) {
...
void FailureArray::BuildFailureArray(
  ...
  BuildOutgoingEdgeLabelsForTrie(failure_vocab_tokens, trie,
                                 &node_outgoing_edge_labels);
  ...
  while (!trie_node_queue.empty()) {
    ...
    for (const char edge_label : outgoing_labels_sorted) {
      ...
      if (trie->TryGetData(child_node, &child_data_value)) {
        ...
        AssignFailureLinkAndPops(child_node.node_id_, failure_link,
                                 {child_data_value},
        ...
        while (true) {
          if (trie->TryTraverseOneStep(&curr_node, edge_label)) {
            AssignFailureLinkAndPops(
                child_node.node_id_, curr_node.node_id_, one_step_pops,
...
void FailureArray::GetFailurePopsAndAppendToOut(
  ...
  utils::GetFailurePopsOffsetAndLength(failure_pops_offset_length, &offset,
                                       &length);
...
void FailureArray::BuildOutgoingEdgeLabelsFromToken(
    const FailureVocabToken& vocab_token, Trie* trie,
    std::vector<std::unordered_set<char>>* node_outgoing_edge_labels) {
