...
class PrefixMatcher {
 public:
  ...
  std::string GlobalReplace(const char* w, size_t w_len, const char* out,
                            size_t out_len, const char** result_w) const;
...
class Normalizer {
 public:
  ...
  virtual bool Normalize(const char* input, size_t input_len,
 ...
 private:
  ...
  void Replace(const simple_string_view& new_part,
               const simple_string_view& old_part, std::vector<int>* changes,
               std::u32string* u32content = nullptr) const;
  ...
  static void DecodePrecompiledCharsMap(const char* blob, size_t blob_size,
