...
std::string PrefixMatcher::GlobalReplace(const char* w, size_t w_len,
...
void Normalizer::Init() {
  if (!precompiled_charsmap_.empty()) {
    ...
    DecodePrecompiledCharsMap(precompiled_charsmap_.data(),
                              precompiled_charsmap_.length(), &trie_blob_,
                              &normalized_blob_, &precompiled_charsmap_buffer_);
#else
    DecodePrecompiledCharsMap(precompiled_charsmap_.data(),
                              precompiled_charsmap_.length(), &trie_blob_,
                              &normalized_blob_);
...
void Normalizer::DecodePrecompiledCharsMap(const char* blob, size_t blob_size,
...
bool Normalizer::Normalize(const char* input, size_t input_len,
                           std::string* normalized,
  ...
  while (iter->next() != icu::BreakIterator::DONE) {
    ...
    if (curr_len < 6) {
      ...
      if (sp.data() != input + curr_pos) {
        ...
        Replace(sp, simple_string_view(input + curr_pos, curr_len),
                norm_to_orig, u32content);
        modified = true;
        curr_pos = next_pos;
        continue;
      }
