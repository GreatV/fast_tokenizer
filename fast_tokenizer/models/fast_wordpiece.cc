...
void FastWordPiece::InitFailureAndTrie() {
  ...
  failure_array_.InitFromVocabAndTrie(vocab_, &trie_, unk_token_,
...
void FastWordPiece::PrecomputeEncodeValueForSubwordPrefix() {
  ...
  for (auto& token : subword_prefix_tokens) {
    utils::FailureVocabToken failure_vocab_token(token.value_, token.id_,
                                                 continuing_subword_prefix_);
...
bool FastWordPiece::TryFollowFailureLinkAndCollectTokens(
    const std::string& sequence, int sequence_offset_in_text,
    int* curr_offset_in_sequence, utils::Trie::TraversalCursor* node,
    std::vector<core::Token>* tokens) const {
  int curr_node_value = 0;
  if (trie_.TryGetData(*node, &curr_node_value)) {
    AppendTokensToOutput(sequence, sequence_offset_in_text,
                         curr_offset_in_sequence, curr_node_value, tokens);
  ...
  utils::GetFailurePopsOffsetAndLength(node_aux->failure_pops_offset_length_,
                                       &offset, &length);
  for (int i = offset; i < offset + length; ++i) {
    AppendTokensToOutput(sequence, sequence_offset_in_text,
                         curr_offset_in_sequence,
                         failure_array_.GetFailurePop(i), tokens);
  }
...
void FastWordPiece::AppendTokensToOutput(
    const std::string& sequence, int sequence_offset_in_text,
    int* curr_offset_in_sequence, int curr_node_value,
    std::vector<core::Token>* tokens) const {
...
void FastWordPiece::ResetOutputAppendUNK(
    int sequence_offset_in_text, int sequence_size, int* original_num_tokens,
    std::vector<core::Token>* tokens) const {
...
bool FastWordPiece::TryHandleContinuingSubWordPrefix(
    const std::string& sequence, int sequence_offset_in_text,
    const utils::Trie::TraversalCursor& curr_node, int* original_num_tokens,
    int* curr_offset_in_sequence, std::vector<core::Token>* tokens) const {
  ...
  if (encoded_value_for_subword_prefix_.size() == 1 &&
      utils::GetTokenIdFromEncodedValue(encoded_value_for_subword_prefix_[0]) ==
          unk_token_id_) {
    ResetOutputAppendUNK(sequence_offset_in_text, sequence.size(),
                         original_num_tokens, tokens);
    return true;
  }
  for (int encoded_token_value : encoded_value_for_subword_prefix_) {
    AppendTokensToOutput(sequence, sequence_offset_in_text,
                         curr_offset_in_sequence, encoded_token_value, tokens);
  }
  return true;
}
...
void FastWordPiece::HandleTheRemainingStringOnTriePath(
    const std::string& sequence, int sequence_offset_in_text,
    utils::Trie::TraversalCursor* curr_node, int* original_num_tokens,
    int* curr_offset_in_sequence, std::vector<core::Token>* tokens) const {
  if (curr_node->node_id_ == utils::Trie::kRootNodeId) {
    return;
  }
  if (TryHandleContinuingSubWordPrefix(sequence, sequence_offset_in_text,
                                       *curr_node, original_num_tokens,
                                       curr_offset_in_sequence, tokens)) {
    *original_num_tokens = tokens->size();
    return;
  }
  while (curr_node->node_id_ != trie_.GetSuffixRoot() &&
         curr_node->node_id_ != trie_.GetPuncFailureNode()) {
    if (!TryFollowFailureLinkAndCollectTokens(sequence, sequence_offset_in_text,
                                              curr_offset_in_sequence,
                                              curr_node, tokens)) {
      ResetOutputAppendUNK(sequence_offset_in_text, sequence.size(),
                           original_num_tokens, tokens);
  ...
  if (unicode_len > max_input_chars_per_word_) {
    ...
    for (auto ch : sequence) {
      while (!trie_.TryTraverseOneStep(&curr_node, ch)) {
        if (!TryFollowFailureLinkAndCollectTokens(sequence, 0,
                                                  &curr_offset_in_sequence,
                                                  &curr_node, &all_tokens)) {
          ResetOutputAppendUNK(0, sequence.size(), &original_num_tokens,
                               &all_tokens);
          return all_tokens;
        }
      }
    }
    HandleTheRemainingStringOnTriePath(sequence, 0, &curr_node,
                                       &original_num_tokens,
                                       &curr_offset_in_sequence, &all_tokens);
  }
  ...
  while (curr_idx < seq_len) {
    ...
    while (curr_idx < seq_len) {
      ...
      while (!trie_.TryTraverseSeveralSteps(&curr_node, curr_substr)) {
        if (!TryFollowFailureLinkAndCollectTokens(
                sequence_substr, word_offset_in_sequence, &curr_offset_in_word,
                &curr_node, &all_tokens)) {
          fail_to_match = true;
          break;
        }
    ...
    if (curr_idx >= seq_len) {
      HandleTheRemainingStringOnTriePath(
          sequence_substr, word_offset_in_sequence, &curr_node,
          &original_num_tokens, &curr_offset_in_word, &all_tokens);
      break;
    }
    ...
    if (curr_unicode_char_is_space ||
      ...
      HandleTheRemainingStringOnTriePath(
          ...
          word_offset_in_sequence, &curr_node, &original_num_tokens,
          &curr_offset_in_word, &all_tokens);
      if (curr_unicode_char_is_space) {
        curr_idx += chwidth;
      }
      continue;
    ...
    ResetOutputAppendUNK(word_offset_in_sequence,
                         ...
                         &original_num_tokens, &all_tokens);
  }
  if (all_tokens.size() == 0) {
    ResetOutputAppendUNK(0, sequence.size(), &original_num_tokens, &all_tokens);
  }
...
FastWordPiece FastWordPiece::GetFastWordPieceFromFile(
    const std::string& file, const std::string& unk_token,
    size_t max_input_chars_per_word,
    const std::string& continuing_subword_prefix, bool with_pretokenization) {
  auto vocab = GetVocabFromFile(file);
  return FastWordPiece(vocab, unk_token, max_input_chars_per_word,
                       continuing_subword_prefix, with_pretokenization);
}
