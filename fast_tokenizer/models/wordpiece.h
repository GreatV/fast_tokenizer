...
struct FASTTOKENIZER_DECL WordPiece : public Model {
  WordPiece();
  WordPiece(const core::Vocab& vocab, const std::string& unk_token = "[UNK]",
            size_t max_input_chars_per_word = 100,
            const std::string& continuing_subword_prefix = "##",
            bool handle_chinese_chars = true);
  // Move version
  WordPiece(core::Vocab&& vocab, std::string&& unk_token,
            size_t max_input_chars_per_word,
            std::string&& continuing_subword_prefix, bool handle_chinese_chars);
