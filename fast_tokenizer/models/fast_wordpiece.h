...
struct FASTTOKENIZER_DECL FastWordPiece : public WordPiece {
  ...
  static FastWordPiece GetFastWordPieceFromFile(
      const std::string& file, const std::string& unk_token = "[UNK]",
      size_t max_input_chars_per_word = 100,
      const std::string& continuing_subword_prefix = "##",
      bool with_pretokenization = false);
 ...
 private:
  ...
  void HandleTheRemainingStringOnTriePath(
      const std::string& sequence, int sequence_offset_in_text,
      utils::Trie::TraversalCursor* node, int* original_num_tokens,
      int* curr_offset_in_sequence, std::vector<core::Token>* tokens) const;
  bool TryHandleContinuingSubWordPrefix(
      const std::string& sequence, int sequence_offset_in_text,
      const utils::Trie::TraversalCursor& node, int* original_num_tokens,
      int* curr_offset_in_sequence, std::vector<core::Token>* tokens) const;
  void ResetOutputAppendUNK(int sequence_offset_in_text, int sequence_size,
