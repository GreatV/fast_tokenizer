...
WordPiece::WordPiece(const core::Vocab& vocab, const std::string& unk_token,
...
// Move version
WordPiece::WordPiece(core::Vocab&& vocab, std::string&& unk_token,
                     size_t max_input_chars_per_word,
...
std::vector<core::Token> WordPiece::Tokenize(const std::string& sequence) {
  ...
  if (unicode_len > max_input_chars_per_word_) {
    all_tokens.emplace_back(vocab_.at(unk_token_), unk_token_,
...
WordPiece WordPieceFactory::CreateWordPieceModel() {
  ...
  return WordPiece{config_.vocab_, config_.unk_token_,
