...
void Unigram::Init(const core::VocabList& vocab,
  ...
  if (trie_->build(sorted_keys.size(), const_cast<char**>(&sorted_keys[0]),
                   nullptr, &sorted_values[0]) != 0) {
    std::ostringstream oss;
  ...
  for (size_t id = 0; id < n; ++id) {
    const int num_nodes =
        trie_->commonPrefixSearch(vocab[id].first.data(), results.data(),
                                  results.size(), vocab[id].first.size());
