...
#define CREATE_PROCESSED_PARI_ENCODING_SEQ(encoding_ptr, attr, name,       \
                                           back_value)                     \
  auto encoding_##name = encoding_ptr->Get##attr();                        \
  ...
  name.back() = back_value
    ...
    CREATE_PROCESSED_PARI_ENCODING_SEQ(pair_encoding, Tokens, tokens,
                                       sep_.first);
    CREATE_PROCESSED_PARI_ENCODING_SEQ(pair_encoding, WordsIdx, word_idx, -1);
    core::Offset empty_offsets = {0, 0};
    CREATE_PROCESSED_PARI_ENCODING_SEQ(pair_encoding, Offsets, offsets,
                                       empty_offsets);
    ...
    for (auto& overflow_pair_encoding : overflowings) {
      CREATE_PROCESSED_PARI_ENCODING_SEQ((&overflow_pair_encoding), Ids, ids,
                                         sep_.second);
      CREATE_PROCESSED_PARI_ENCODING_SEQ((&overflow_pair_encoding), TypeIds,
                                         type_ids, 1);
      CREATE_PROCESSED_PARI_ENCODING_SEQ((&overflow_pair_encoding), Tokens,
                                         tokens, sep_.first);
      CREATE_PROCESSED_PARI_ENCODING_SEQ((&overflow_pair_encoding), WordsIdx,
                                         word_idx, -1);
      core::Offset empty_offsets = {0, 0};
      CREATE_PROCESSED_PARI_ENCODING_SEQ((&overflow_pair_encoding), Offsets,
