...
void to_json(nlohmann::json& j, const SpecialTokensMap& tokens_map) {
  for (auto it = tokens_map.tokens_map_.begin();
       it != tokens_map.tokens_map_.end(); ++it) {
...
TemplatePostProcessor::TemplatePostProcessor(
    const Template& single, const Template& pair,
...
void TemplatePostProcessor::ApplyTemplate(
    const Template& pieces, core::Encoding* encoding,
    core::Encoding* pair_encoding, bool add_special_tokens,
  ...
  for (auto& overflow_encoding : overflowings) {
    ...
    if (pair_encoding != nullptr) {
      ...
      ApplyTemplate(pieces, &encoding_copy, &pair_encoding_copy,
                    add_special_tokens, &result_overflowing_encoding);
      ...
      ApplyTemplate(pieces, &encoding_copy, pair_encoding, add_special_tokens,
  ...
  for (auto& piece : pieces.pieces_) {
    if (paddlenlp::get_if<TemplateSequence>(&piece) != nullptr) {
      ...
      if (template_sequence.first == SequenceType::SEQ_A) {
        ...
        ids.insert(ids.end(), encoding->GetIds().begin(),
                   encoding->GetIds().end());
        type_ids.insert(type_ids.end(), encoding->GetLen(),
                        template_sequence.second);
        tokens.insert(tokens.end(), encoding->GetTokens().begin(),
                      encoding->GetTokens().end());
        words_idx.insert(words_idx.end(), encoding->GetWordsIdx().begin(),
                         encoding->GetWordsIdx().end());
        offsets.insert(offsets.end(), encoding->GetOffsets().begin(),
        ...
        sequence_ranges[0] = {seq_start, seq_end};
        ids.insert(ids.end(), pair_encoding->GetIds().begin(),
                   pair_encoding->GetIds().end());
        type_ids.insert(type_ids.end(), pair_encoding->GetLen(),
                        template_sequence.second);
        tokens.insert(tokens.end(), pair_encoding->GetTokens().begin(),
                      pair_encoding->GetTokens().end());
        words_idx.insert(words_idx.end(), pair_encoding->GetWordsIdx().begin(),
                         pair_encoding->GetWordsIdx().end());
        offsets.insert(offsets.end(), pair_encoding->GetOffsets().begin(),
  ...
  *result_encoding = core::Encoding(
      std::move(ids), std::move(type_ids), std::move(tokens),
      std::move(words_idx), std::move(offsets), std::move(special_tokens_mask),
      std::move(attention_mask), std::move(result_overflowings),
...
void TemplatePostProcessor::operator()(core::Encoding* encoding,
                                       ...
                                       core::Encoding* result_encoding) const {
  if (pair_encoding != nullptr) {
    ApplyTemplate(pair_, encoding, pair_encoding, add_special_tokens,
                  result_encoding);
  } else {
    ApplyTemplate(single_, encoding, pair_encoding, add_special_tokens,
                  result_encoding);
  }
}
