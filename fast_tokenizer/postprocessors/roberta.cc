...
RobertaPostProcessor::RobertaPostProcessor(
    ...
    const std::pair<std::string, uint32_t>& cls, bool trim_offsets,
...
#define CREATE_PROCESSED_ENCODING_SEQ(encoding_ptr, attr, name, head_value,    \
                                      back_value)                              \
  ...
  // tokens
  CREATE_PROCESSED_ENCODING_SEQ(encoding, Tokens, tokens, cls_.first,
                                sep_.first);
  // word_idx
  CREATE_PROCESSED_ENCODING_SEQ(encoding, WordsIdx, word_idx, -1, -1);
  // offsets
  core::Offset empty_offsets = {0, 0};
  CREATE_PROCESSED_ENCODING_SEQ(encoding, Offsets, offsets, empty_offsets,
                                empty_offsets);
  // special_tokens_mask
  ...
  for (auto& overflow_encoding : overflowings) {
    CREATE_PROCESSED_ENCODING_SEQ((&overflow_encoding), Ids, ids, cls_.second,
                                  sep_.second);
    CREATE_PROCESSED_ENCODING_SEQ((&overflow_encoding), TypeIds, type_ids, 0,
                                  0);
    CREATE_PROCESSED_ENCODING_SEQ((&overflow_encoding), Tokens, tokens,
                                  cls_.first, sep_.first);
    CREATE_PROCESSED_ENCODING_SEQ((&overflow_encoding), WordsIdx, word_idx, -1,
                                  -1);
    CREATE_PROCESSED_ENCODING_SEQ((&overflow_encoding), Offsets, offsets,
                                  empty_offsets, empty_offsets);
    ...
    overflow_encoding = std::move(core::Encoding(
        std::move(ids), std::move(type_ids), std::move(tokens),
        std::move(word_idx), std::move(offsets), std::move(special_tokens_mask),
        std::move(attention_mask),
        std::vector<core::Encoding>(),  // No overflowing
        std::move(sequence_ranges)));
  }

  core::Encoding new_encoding(
      std::move(ids), std::move(type_ids), std::move(tokens),
      std::move(word_idx), std::move(offsets), std::move(special_tokens_mask),
      std::move(attention_mask), std::move(overflowings),
      std::move(sequence_ranges));

  // Construct the sequence as: [CLS] A [SEP] [SEP] B [SEP]
  if (pair_encoding != nullptr) {
    // ids
    CREATE_PROCESSED_ENCODING_SEQ(pair_encoding, Ids, ids, sep_.second,
                                  sep_.second);
    ...
    // tokens
    CREATE_PROCESSED_ENCODING_SEQ(pair_encoding, Tokens, tokens, sep_.first,
                                  sep_.first);
    // word_idx
    CREATE_PROCESSED_ENCODING_SEQ(pair_encoding, WordsIdx, word_idx, -1, -1);
    // offsets
    core::Offset empty_offsets = {0, 0};
    CREATE_PROCESSED_ENCODING_SEQ(pair_encoding, Offsets, offsets,
                                  empty_offsets, empty_offsets);
    // special_tokens_mask
    std::vector<uint32_t> special_tokens_mask(ids.size(), 0);
    special_tokens_mask.front() = special_tokens_mask.back() = 1;
    ...
    for (auto& overflow_pair_encoding : overflowings) {
      // ids
      CREATE_PROCESSED_ENCODING_SEQ((&overflow_pair_encoding), Ids, ids,
                                    sep_.second, sep_.second);
      // type_ids
      std::vector<uint32_t> type_ids(
          overflow_pair_encoding.GetTypeIds().size() + 2, 0);
      // tokens
      CREATE_PROCESSED_ENCODING_SEQ((&overflow_pair_encoding), Tokens, tokens,
                                    sep_.first, sep_.first);
      // word_idx
      CREATE_PROCESSED_ENCODING_SEQ((&overflow_pair_encoding), WordsIdx,
                                    word_idx, -1, -1);
      // offsets
      core::Offset empty_offsets = {0, 0};
      CREATE_PROCESSED_ENCODING_SEQ((&overflow_pair_encoding), Offsets, offsets,
                                    empty_offsets, empty_offsets);
      // special_tokens_mask
      std::vector<uint32_t> special_tokens_mask(ids.size(), 0);
      special_tokens_mask.front() = special_tokens_mask.back() = 1;
      ...
      std::unordered_map<uint32_t, core::Range> sequence_ranges;
      sequence_ranges[0] = {1, ids.size() - 1};
      overflow_pair_encoding = std::move(core::Encoding(
          std::move(ids), std::move(type_ids), std::move(tokens),
          std::move(word_idx), std::move(offsets),
          std::move(special_tokens_mask), std::move(attention_mask),
          std::vector<core::Encoding>(),  // No overflowing
          std::move(sequence_ranges)));
    }
    core::Encoding new_pair_encoding(
        std::move(ids), std::move(type_ids), std::move(tokens),
        std::move(word_idx), std::move(offsets), std::move(special_tokens_mask),
        std::move(attention_mask), std::move(overflowings),
        std::move(sequence_ranges));
