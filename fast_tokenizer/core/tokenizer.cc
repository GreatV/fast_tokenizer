...
void Tokenizer::EnableTruncMethod(size_t max_len, size_t stride,
                                  Direction direction, TruncStrategy strategy) {
...
void Tokenizer::EnablePadMethod(Direction direction, uint32_t pad_id,
                                uint32_t pad_type_id,
                                const std::string& pad_token, uint32_t* length,
...
bool Tokenizer::DoTokenize(pretokenizers::PreTokenizedString* pretokenized,
                           ...
                           OffsetType offset_type, Encoding* encoding) const {
  pretokenized->Tokenize([&](normalizers::NormalizedString* normalized) {
    return this->GetModelPtr()->Tokenize(normalized->GetStr());
  });
  return pretokenized->TransformToEncoding(word_idx, type_id, offset_type,
                                           encoding);
}
...
struct InputStringVisitor {
  InputStringVisitor(const Tokenizer* tokenizer, uint32_t type_id,
                     OffsetType offset_type, Encoding* encodings)
      : tokenizer_(tokenizer),
        type_id_(type_id),
        offset_type_(offset_type),
        encodings_(encodings) {}
  void operator()(const std::vector<std::string>& pretokenized_texts) const {
    tokenizer_->EncodeSingleText(pretokenized_texts, type_id_, offset_type_,
                                 encodings_);
  }
...
void Tokenizer::EncodeSingleString(const InputString& input_string,
                                   uint32_t type_id, OffsetType offset_type,
                                   Encoding* encodings) const {
  paddlenlp::visit(InputStringVisitor(this, type_id, offset_type, encodings),
                   input_string);
}

void Tokenizer::PostProcess(Encoding* encoding, Encoding* pair_encoding,
  ...
  // 2. Post process
  if (post_processor_ == nullptr) {
    postprocessors::PostProcessor::DefaultProcess(encoding, pair_encoding,
                                                  result_encoding);
  } else {
    (*post_processor_)(encoding, pair_encoding, add_special_tokens,
                       result_encoding);
  }
  // 3. Pad
  if (use_padding_) {
...
void Tokenizer::EncodePairStrings(const EncodeInput& encode_input,
  ...
  if (encode_input.type() == typeid(InputString)) {
    ...
    EncodeSingleString(input_string_pair.second, 1, OffsetType::CHAR,
                       &pair_encoding);
...
void Tokenizer::MultiThreadEncodeBatchStrings(
    ...
    std::vector<Encoding>* encodings, bool add_special_tokens,
    size_t start_index, size_t step_index) const {
  ...
  for (size_t i = start_index; i < end_index; ++i) {
    EncodePairStrings(texts[i], text_pairs[i], &(*encodings)[i],
                      add_special_tokens);
  }
}
...
void Tokenizer::MultiThreadEncodeBatchStrings(
  ...
  if (end_index > batch_size) end_index = batch_size;
  for (size_t i = start_index; i < end_index; ++i) {
    EncodePairStrings(batch_encode_input[i], &(*encodings)[i],
                      add_special_tokens);
  }
}

void Tokenizer::MultiThreadEncodeBatchStrings(
    const std::vector<std::string>& texts, std::vector<Encoding>* encodings,
    bool add_special_tokens, size_t start_index, size_t step_index) const {
  auto batch_size = texts.size();
  size_t end_index = start_index + step_index;
  if (end_index > batch_size) end_index = batch_size;
  for (size_t i = start_index; i < end_index; ++i) {
...
void Tokenizer::EncodeBatchStrings(
  ...
  auto func = [&](size_t start_index, size_t step_index) {
    MultiThreadEncodeBatchStrings(batch_encode_input, encodings,
                                  add_special_tokens, start_index, step_index);
  };
...
void Tokenizer::EncodeBatchStrings(const std::vector<std::string>& texts,
  ...
  encodings->resize(batch_size);
  auto func = [&](size_t start_index, size_t step_index) {
    MultiThreadEncodeBatchStrings(texts, encodings, add_special_tokens,
...
void Tokenizer::EncodeBatchStrings(const std::vector<std::string>& texts,
  ...
  encodings->resize(batch_size);
  auto func = [&](size_t start_index, size_t step_index) {
    MultiThreadEncodeBatchStrings(texts, text_pairs, encodings,
                                  add_special_tokens, start_index, step_index);
  };
  RunMultiThread(func, batch_size);
...
void Tokenizer::EncodeSingleText(
    const std::vector<std::string>& pretokenized_texts, uint32_t type_id,
    OffsetType offset_type, Encoding* encoding) const {
  std::vector<Encoding> encodings;
...
void Tokenizer::EncodeSingleText(const std::string& raw_text, uint32_t type_id,
...
void Tokenizer::Decode(const std::vector<uint32_t>& token_ids,
                       std::string* result, bool skip_special_tokens) const {
  // Get tokens
...
void Tokenizer::MultiThreadDecodeBatch(
    ...
    std::vector<std::string>* results, bool skip_special_tokens,
    size_t start_index, size_t step_index) const {
...
void Tokenizer::DecodeBatch(
    const std::vector<std::vector<uint32_t>>& batch_token_ids,
    std::vector<std::string>* results, bool skip_special_tokens) const {
  auto batch_size = batch_token_ids.size();
  results->resize(batch_size);
  auto func = [&](size_t start_index, size_t step_index) {
    MultiThreadDecodeBatch(batch_token_ids, results, skip_special_tokens,
                           start_index, step_index);
  };
  RunMultiThread(func, batch_size);
}
