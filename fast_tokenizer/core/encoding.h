...
class FASTTOKENIZER_DECL Encoding {
 public:
  ...
  // Move version
  Encoding(std::vector<uint32_t>&& ids, std::vector<uint32_t>&& type_ids,
  ...
  void Pad(uint32_t target_length, uint32_t pad_id, uint32_t pad_type_id,
           const std::string& pad_token, Direction direction);
  // Static method
