...
class FASTTOKENIZER_DECL Tokenizer {
 public:
  ...
  void EnablePadMethod(Direction direction, uint32_t pad_id,
                       uint32_t pad_type_id, const std::string& pad_token,
                       uint32_t* length, uint32_t* pad_to_multiple_of);
  ...
  bool DoTokenize(pretokenizers::PreTokenizedString* pretokenized,
                  uint32_t type_id, const std::vector<uint32_t>& word_idx,
                  OffsetType offset_type, Encoding* encoding) const;
  bool DoPreTokenize(pretokenizers::PreTokenizedString* pretokenized) const;

  void EncodeSingleString(const InputString& input_string, uint32_t type_id,
                          OffsetType offset_type, Encoding* encodings) const;
  void PostProcess(Encoding* encoding, Encoding* pair_encoding,
                   bool add_special_tokens, Encoding* result_encoding) const;
  void EncodePairStrings(const EncodeInput& encode_input, Encoding* encodings,
                         bool add_special_tokens = true) const;
  void EncodePairStrings(const std::string& text, const std::string& text_pair,
                         Encoding* encodings,
                         bool add_special_tokens = true) const;
  ...
  void EncodeSingleText(const std::string& raw_text, uint32_t type_id,
                        OffsetType offset_type, Encoding* encodings) const;
  ...
  void Decode(const std::vector<uint32_t>& token_ids, std::string* result,
