...
struct FASTTOKENIZER_DECL BPEWord {
  ...
  void Merge(uint32_t c1, uint32_t c2, uint32_t replacement,
    ...
    for (int i = 0; i < symbols_.size(); ++i) {
      ...
      if (symbols_[i].ch_ == c1 && i + 1 < symbols_.size() &&
        ...
        Symbol symbols{replacement, first.prev_, second.next_,
                       first.len_ + second.len_};
  ...
  void MergeAll(const MergeMap& merges, const std::vector<float>& dropout) {
    ...
    for (int i = 0; i < symbols_.size() - 1; ++i) {
      ...
      if (merges.find({first.ch_, second.ch_}) != merges.end()) {
        ...
        core::Merge new_merge{static_cast<size_t>(i), new_merge_info.first,
      ...
      if (can_skip && distrib(gen) < dropout[0]) {
        ...
        if (current.prev_ >= 0) {
          ...
          if (merges.find(new_pair) != merges.end()) {
            ...
            queue.push({static_cast<size_t>(current.prev_), new_merge.first,
                        new_merge.second});
          }
        }
