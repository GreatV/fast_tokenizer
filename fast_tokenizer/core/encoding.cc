...
// Move version
Encoding::Encoding(
    std::vector<uint32_t>&& ids, std::vector<uint32_t>&& type_ids,
    std::vector<std::string>&& tokens, std::vector<uint32_t>&& words_idx,
    std::vector<Offset>&& offsets, std::vector<uint32_t>&& special_tokens_mask,
    std::vector<uint32_t>&& attention_mask, std::vector<Encoding>&& overflowing,
...
void Encoding::Truncate(size_t max_len, size_t stride, Direction direction) {
  ...
  if (max_len < encoding_len) {
    ...
    Encoding new_encoding(
        ...
        std::vector<Encoding>(), std::unordered_map<uint32_t, Range>());
    // Set overflowing
...
void Encoding::Pad(uint32_t target_length, uint32_t pad_id,
  ...
  if (GetLen() < target_length) {
    ...
    if (direction == LEFT) {
      ...
      words_idx_.insert(words_idx_.begin(), pad_len,
      ...
      words_idx_.insert(words_idx_.end(), pad_len,
                        std::numeric_limits<uint32_t>::max());
...
bool TruncateEncodings(Encoding* encoding, Encoding* pair_encoding,
  ...
  if (method.strategy_ == TruncStrategy::LONGEST_FIRST) {
    if (pair_encoding == nullptr) {
      ...
      pair_encoding->Truncate(pair_encoding_len, method.stride_,
                              method.direction_);
    }
  } else {
    ...
    if (result->GetIds().size() > num_of_removed_ids) {
      result->Truncate(result->GetIds().size() - num_of_removed_ids,
                       method.stride_, method.direction_);
    } else {
...
void MultiThreadPadEncodings(std::vector<Encoding>* encodings,
                             const PadMethod& method, size_t pad_length,
                             size_t start_index, size_t step_index) {
  ...
  for (size_t i = start_index; i < end_index; ++i) {
    ...
    encoding.Pad(pad_length, method.pad_id_, method.pad_token_type_id_,
                 method.pad_token_, method.direction_);
  }
}
void PadEncodings(std::vector<Encoding>* encodings, const PadMethod& method) {
  ...
  auto batch_size = encodings->size();
  auto func =
      std::bind(&MultiThreadPadEncodings, encodings, std::ref(method),
                pad_length, std::placeholders::_1, std::placeholders::_2);
