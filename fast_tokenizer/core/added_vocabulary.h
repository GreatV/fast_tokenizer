...
class FASTTOKENIZER_DECL AddedToken {
 public:
  AddedToken();
  AddedToken(const std::string& content, bool is_special = false,
...
class FASTTOKENIZER_DECL AddedVocabulary {
 public:
  ...
  bool TokenToId(const std::string& token, const models::Model& model,
                 uint32_t* id) const;
  bool IdToToken(uint32_t id, const models::Model& model,
  ...
  void RefreshAddedTokens(const models::Model& model,
                          const normalizers::Normalizer* normalizers);
  bool FindMatch(const std::string& sequence, const MatchSet& pattern,
