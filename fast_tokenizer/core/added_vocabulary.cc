...
inline size_t GetEndSpaceIdx(const std::string& sequence) {
  ...
  pattern.Match(sequence, 0, sequence.length(), RE2::UNANCHORED, &result_str,
                1);
...
AddedToken::AddedToken(const std::string& content, bool is_special,
                       bool single_word, bool lstrip, bool rstrip)
    : content_(content),
...
bool AddedVocabulary::IdToToken(uint32_t id, const models::Model& model,
...
size_t AddedVocabulary::AddSpecialTokens(
    const std::vector<AddedToken>& tokens, const models::Model& model,
...
bool AddedVocabulary::FindMatch(const std::string& sequence,
  ...
  while (pattern.first->Match(sequence, start, end, RE2::UNANCHORED,
                              &result_str, 1) &&
         result_str != "") {
    ...
    uint32_t id = pattern.second.at(std::string(result_str));
...
bool AddedVocabulary::SplitWithIndices(
    const normalizers::NormalizedString& normalized, const MatchSet& pattern,
...
void AddedVocabulary::ExtractAndNormalize(
    const normalizers::Normalizer* normalizers, const std::string& sequence,
    pretokenizers::PreTokenizedString* pretokenized) const {
  pretokenized->SetOriginalStr(sequence);
  pretokenized->Split(
      [&](int idx, normalizers::NormalizedString* normalized,
  ...
  pretokenized->Split(
      [&](int idx, normalizers::NormalizedString* normalized,
        ...
        if (normalizers != nullptr) {
          ...
          this->SplitWithIndices(*normalized, this->split_normalized_trie_,
                                 string_splits);
        }
      });
}
