...
ClipFastTokenizer::ClipFastTokenizer(
    ...
    uint32_t max_length, const std::string& unk_token,
    const std::string& pad_token, const std::string& bos_token,
    const std::string& eos_token, bool add_prefix_space,
    const std::string& continuing_subword_prefix,
    const std::string& end_of_word_suffix, bool trim_offsets) {
  core::Vocab vocab;
  core::Merges merges;
  models::BPE::GetVocabAndMergesFromFile(vocab_path, merges_path, &vocab,
                                         &merges);
  VLOG(6) << "The vocab size of ClipFastTokenizer is " << vocab.size();
  VLOG(6) << "The merges size of ClipFastTokenizer is " << merges.size();

  models::BPE bpe(vocab, merges, 10000, {}, {unk_token},
                  {continuing_subword_prefix}, {end_of_word_suffix}, false);
  // Set tokenizer model
  this->SetModel(bpe);
  ...
  postprocessors::RobertaPostProcessor roberta_postprocessor(
      {eos_token, eos_token_id_}, {bos_token, bos_token_id_},
      /* trim_offsets= */ false, add_prefix_space);
  this->SetPostProcessor(roberta_postprocessor);

  if (max_length == 0) {
    this->DisableTruncMethod();
  } else {
    this->EnableTruncMethod(max_length, 0, core::Direction::RIGHT,
