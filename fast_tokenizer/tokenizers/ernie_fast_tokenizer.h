...
struct FASTTOKENIZER_DECL ErnieFastTokenizer : public core::Tokenizer {
 ...
 private:
  void Init(const core::Vocab& vocab, const std::string& unk_token = "[UNK]",
            const std::string& sep_token = "[SEP]",
            const std::string& cls_token = "[CLS]",
            const std::string& pad_token = "[PAD]",
            const std::string& mask_token = "[MASK]", bool clean_text = true,
            bool handle_chinese_chars = true, bool strip_accents = true,
            bool lowercase = true, const std::string& wordpieces_prefix = "##",
            uint32_t max_sequence_len = 0);
};
