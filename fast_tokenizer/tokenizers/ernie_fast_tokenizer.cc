...
ErnieFastTokenizer::ErnieFastTokenizer(
    const std::string& vocab_path, const std::string& unk_token,
    const std::string& sep_token, const std::string& cls_token,
    const std::string& pad_token, const std::string& mask_token,
    bool clean_text, bool handle_chinese_chars, bool strip_accents,
    bool lowercase, const std::string& wordpieces_prefix,
  ...
  Init(vocab, unk_token, sep_token, cls_token, pad_token, mask_token,
       clean_text, handle_chinese_chars, strip_accents, lowercase,
       wordpieces_prefix, max_sequence_len);
}

ErnieFastTokenizer::ErnieFastTokenizer(
    const core::Vocab& vocab, const std::string& unk_token,
    const std::string& sep_token, const std::string& cls_token,
    const std::string& pad_token, const std::string& mask_token,
    bool clean_text, bool handle_chinese_chars, bool strip_accents,
    bool lowercase, const std::string& wordpieces_prefix,
...
void ErnieFastTokenizer::Init(
    ...
    bool lowercase, const std::string& wordpieces_prefix,
    uint32_t max_sequence_len) {
  models::FastWordPiece wordpiece(vocab, unk_token,
                                  100 /* max_input_chars_per_word */,
                                  wordpieces_prefix, true);
  ...
  normalizers::BertNormalizer bert_normalizer(clean_text, handle_chinese_chars,
  ...
  if (max_sequence_len == 0) {
    ...
    this->EnableTruncMethod(max_sequence_len, 0, core::Direction::RIGHT,
