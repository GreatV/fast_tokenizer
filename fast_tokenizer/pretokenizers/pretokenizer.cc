...
void PreTokenizedString::Split(
    std::function<void(int, normalizers::NormalizedString*,
...
bool PreTokenizedString::TransformToEncoding(
    const std::vector<uint32_t>& input_word_idx, uint32_t type_id,
    core::OffsetType offset_type, core::Encoding* encoding) const {
  if (splits_.empty()) {
  ...
  return TransformToEncodingUseConvertor<OffsetConverter>(input_word_idx,
                                                          type_id, encoding);
}
...
bool PreTokenizedString::TransformToEncodingUseConvertor(
  ...
  if (input_word_idx.size() == 0) {
    ...
    for (uint32_t i = 0; i < splits_.size(); ++i) {
      std::fill_n(words_idx.begin() + word_offset, splits_[i].tokens_.size(),
                  i);
  ...
  *encoding = std::move(core::Encoding(
      ...
      std::move(tokens), std::move(words_idx), std::move(offsets),
