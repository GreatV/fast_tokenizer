...
fast_tokenizer::tokenizers_impl::ClipFastTokenizer CreateClipFastTokenizer(
    const std::string& vocab_path, const std::string& merge_path,
    uint32_t max_length, bool pad_to_max_length = true) {
  ...
  if (pad_to_max_length) {
    tokenizer.EnablePadMethod(fast_tokenizer::core::RIGHT,
                              tokenizer.GetPadTokenId(), 0,
                              tokenizer.GetPadToken(), &max_length, nullptr);
  }
  return tokenizer;
}
...
int main() {
  ...
  auto tokenizer = CreateClipFastTokenizer("clip_vocab.json", "clip_merges.txt",
                                           /*max_length = */ 77,
